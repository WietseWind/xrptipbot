...
try {
    ...
    if(!empty($req)){
        ...
        $att = ((float)$req['amount']).':'.$req['from_wallet'].':'.$req['to_wallet'].':'.((int)$req['destination_tag']).':'.((int)$req['source_tag']).':'.(@strtoupper(@bin2hex(@utf8_decode($req['memo'])))).':'.((int)$req['escrowts']);
        echo "Processing...";
